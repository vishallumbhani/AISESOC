  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Edge Detail</p>
        <button onClick={onClose}><FiX className="w-3.5 h-3.5 text-gray-600" /></button>
      </div>
      <div className="flex items-center space-x-2">
        <span className="h-0.5 w-6 inline-block rounded" style={{ background: color }} />
        <span className="text-xs font-bold" style={{ color }}>{label}</span>
        {edge.decision && (
          <span className={`ml-auto text-xs font-bold px-2 py-0.5 rounded-full ${isAllow ? "bg-green-900 text-green-400" : "bg-red-900 text-red-400"}`}>
      ...
      <div className="space-y-2 text-xs">
        {[
          ...
          <div key={k} className="flex justify-between">
            ...
            <span className="text-gray-200 font-medium truncate max-w-[160px] text-right">{v}</span>
          </div>
        ) : null)}
        {edge.prompt_preview && (
          <div>
            <p className="text-gray-500 mb-1">Prompt</p>
            <p className="text-gray-300 bg-gray-800 rounded p-2 font-mono text-xs leading-relaxed break-words">
              "{edge.prompt_preview}"
            </p>
          </div>
        )}
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wide">{node.label}</p>
      ...
      {drillLoading ? (
        ...
        <div className="max-h-[calc(100vh-300px)] overflow-y-auto">
          ...
          {/* Risk score */}
          {drillDown.risk_score && (
            <div className="px-4 py-3 border-b border-gray-800">
              <p className="text-xs text-gray-500 font-medium mb-2 flex items-center space-x-1">
                <FiShield className="w-3 h-3" /><span>Risk Score</span>
              </p>
              <div className="flex items-center space-x-3">
                <p className="text-2xl font-bold text-white">{drillDown.risk_score.score.toFixed(1)}</p>
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${SEV_COLOR[drillDown.risk_score.severity] || "text-gray-400 bg-gray-800"}`}>
                  {drillDown.risk_score.severity.toUpperCase()}
                </span>
              </div>
            ...
            <div className="px-4 py-3 border-b border-gray-800">
              ...
              <div className="flex space-x-4">
                {drillDown.event_counts.allow !== undefined && (
                  <div>
                    <p className="text-green-400 font-bold text-lg">{drillDown.event_counts.allow || 0}</p>
                    <p className="text-gray-500 text-xs">Allowed</p>
                  </div>
                )}
                {drillDown.event_counts.deny !== undefined && (
                  <div>
                    <p className="text-red-400 font-bold text-lg">{drillDown.event_counts.deny || 0}</p>
              ...
              <div className="space-y-1">
                {drillDown.related_policies.map((p) => (
                  <div key={p.id} className="flex items-center justify-between text-xs">
                    ...
                    <span className={`font-bold ml-2 ${p.effect === "allow" ? "text-green-400" : "text-red-400"}`}>
          ...
          {drillDown.open_incidents && drillDown.open_incidents.length > 0 && (
            <div className="px-4 py-3 border-b border-gray-800">
              <p className="text-xs text-gray-500 font-medium mb-2 flex items-center space-x-1">
                ...
                <span className="text-red-400">Open Incidents ({drillDown.open_incidents.length})</span>
              </p>
              <div className="space-y-1.5">
                {drillDown.open_incidents.map((inc) => (
                  <div key={inc.id} className="text-xs bg-red-900/20 border border-red-900/30 rounded-md p-2">
                    <p className="text-gray-300 line-clamp-2">{inc.description}</p>
                    <span className="text-red-400 font-medium capitalize">{inc.severity}</span>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div key={ev.id} className="flex items-start space-x-2 text-xs">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex justify-between">
                        <span className="text-gray-300 capitalize">{ev.action}</span>
                        <span className={`font-bold ${ev.status === "allow" ? "text-green-400" : "text-red-400"}`}>
                          {ev.status?.toUpperCase()}
                        </span>
                      </div>
                      {ev.prompt_preview && (
                        <p className="text-gray-600 font-mono truncate">"{ev.prompt_preview}"</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="p-4 text-gray-600 text-xs">No drill-down data available.</div>
      )}
    </div>
  );
}
        ...
        <div className="max-w-[1400px] mx-auto px-4">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-0.5">End User → Agent → Asset → Policy</p>
            </div>
            <button
          ...
          {/* Filter bar */}
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 mb-4 flex flex-wrap gap-3 items-center">
            <span className="text-xs text-gray-500 font-medium uppercase tracking-wide flex-shrink-0">Filters</span>
            {[
            ...
            {activeFilters && (
              <button
                ...
                className="ml-auto text-xs text-indigo-400 hover:text-indigo-300 flex items-center space-x-1"
              >
          ...
          {/* Layer labels */}
          <div className="bg-gray-900 border border-gray-800 rounded-t-xl px-4 pt-3 flex"
          ...
          <div className="flex gap-4">
            {/* Canvas */}
            <div className="flex-1 bg-gray-900 border border-gray-800 border-t-0 rounded-b-xl overflow-auto relative min-h-[480px]">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-[480px] text-gray-600">
                  ...
                  <p className="text-sm">Click <span className="text-indigo-400">Sync from DB</span> to populate</p>
                </div>
              ) : (
              ...
              {!loading && layoutNodes.length > 0 && (
                <div className="absolute bottom-3 left-3 flex space-x-4 text-xs text-gray-600">
            ...
            <div className="w-72 flex-shrink-0 space-y-3">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wide mb-2">Node Types</p>
                ...
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wide mb-2">Edge Types</p>
