...
const ROLE_CLS: Record<string, string> = {
  org_admin:          "text-red-300 bg-red-900/30 border-red-700",
  admin:              "text-red-300 bg-red-900/30 border-red-700",
  security_architect: "text-orange-300 bg-orange-900/30 border-orange-700",
  security_analyst:   "text-blue-300 bg-blue-900/30 border-blue-700",
  auditor:            "text-purple-300 bg-purple-900/30 border-purple-700",
  read_only:          "text-gray-400 bg-gray-800 border-gray-600",
  user:               "text-gray-400 bg-gray-800 border-gray-600",
};
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-950 py-8">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-500 text-xs mt-0.5">Manage organization members and roles</p>
              </div>
            </div>
          ...
          {success && (
            <div className="mb-4 flex items-center gap-2 bg-green-900/30 border border-green-700 text-green-300 rounded-xl px-4 py-3 text-sm">
          ...
          {inviting && (
            <form onSubmit={invite}
              className="bg-gray-900 border border-indigo-700/60 rounded-xl p-5 mb-5 space-y-4">
              <h3 className="font-semibold text-white flex items-center gap-2">
                <FiShield className="w-4 h-4 text-indigo-400" /> Invite New User
              </h3>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setInviting(false)}
                  className="bg-gray-800 border border-gray-700 text-gray-300 hover:text-white px-4 py-2 rounded-lg text-sm">
                  Cancel
                </button>
              </div>
            </form>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-800">
                  {users.length === 0 ? (
                    ...
                    users.map(u => (
                      <tr key={u.id} className={`hover:bg-gray-800/30 transition-colors ${!u.is_active ? "opacity-50" : ""}`}>
                          ...
                          {editingId === u.id ? (
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={() => updateRole(u.id, editRole)}
                                className="text-green-400 hover:text-green-300 text-xs px-2 py-1 bg-green-900/30 rounded">Save</button>
                              <button onClick={() => setEditingId(null)} className="text-gray-500 hover:text-white">
                                <FiX className="w-3.5 h-3.5" />
                              </button>
                            </div>
                          ) : (
                            <span className={`text-xs px-2 py-0.5 rounded-full border font-medium capitalize ${ROLE_CLS[u.role] || "text-gray-400 bg-gray-800 border-gray-600"}`}>
                              {(u.role || "user").replace(/_/g, " ")}
                            </span>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          <span className={`text-xs font-semibold ${u.is_active ? "text-green-400" : "text-red-400"}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className={`p-1.5 rounded hover:bg-gray-800 ${u.is_active ? "text-gray-500 hover:text-orange-400" : "text-gray-500 hover:text-green-400"}`}>
                              {u.is_active
