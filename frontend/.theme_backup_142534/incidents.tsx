...
const STATUS_STYLE: Record<string,string> = {
  ...
  investigating:  "bg-orange-900/40 text-orange-400 border border-orange-700",
  ...
  return (
    <div className="relative flex gap-3">
      ...
      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${
        ...
        isSystem  ? "bg-gray-800 border border-gray-600" :
                    "bg-indigo-900/60 border border-indigo-500"
      }`}>
        {isCreated ? <FiAlertTriangle className="w-3.5 h-3.5 text-red-400" /> :
         isSystem  ? <FiActivity className="w-3.5 h-3.5 text-gray-400" /> :
                     <FiUser className="w-3.5 h-3.5 text-indigo-400" />}
      </div>
      <div className="flex-1 pb-4">
        <div className="flex items-center gap-2 mb-0.5">
          <span className={`text-sm font-semibold ${isCreated ? "text-red-400" : "text-white"}`}>
            {entry.action?.replace(/_/g," ")}
          </span>
          <span className="text-xs text-gray-500 ml-auto font-mono">{fmt(entry.ts)}</span>
        </div>
        <p className="text-xs text-gray-400">{entry.actor}</p>
        {entry.note && <p className="text-xs text-gray-300 mt-1 italic">"{entry.note}"</p>}
      </div>
    </div>
  );
}
...
function InfoRow({ label, value, mono=false }: { label:string; value?:string|null; mono?:boolean }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2 border-b border-gray-800 last:border-0">
      <span className="text-gray-400 text-sm min-w-[120px] flex-shrink-0">{label}</span>
      <span className={`text-white text-sm break-all ${mono ? "font-mono text-xs" : ""}`}>{value}</span>
    </div>
  );
}
...
function InvestigationDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-3xl bg-gray-950 border-l border-gray-700 h-full overflow-y-auto flex flex-col">
        {/* Header */}
        <div className="px-6 py-5 border-b border-gray-700 flex-shrink-0">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Incident Investigation</h2>
                <p className="text-gray-400 text-sm">{incident.incident_type?.replace(/_/g," ")}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {[
              ...
              <div key={c.label} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 flex items-center gap-2">
                <span>{c.icon}</span>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {tab === "overview" && (
            <div className="space-y-4">
              <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 space-y-3">
                <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wide">Update Incident</h3>
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Add Timeline Note</label>
              ...
              <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
                <h3 className="text-xs text-gray-400 uppercase font-semibold mb-3">Incident Details</h3>
          ...
          {tab === "events" && (
            ...
            (inv?.what?.events || []).length === 0
              ? <p className="text-gray-400 text-sm">No runtime events in the incident window.</p>
              : (
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-bold text-xs uppercase ${
                            e.decision === "deny" ? "text-red-400" : "text-green-400"
                          }`}>{e.decision}</span>
                          <span className="text-gray-400 text-xs capitalize">{e.action}</span>
                          {e.end_user?.email && (
                        ...
                        <span className="text-gray-500 text-xs font-mono">{fmt(e.ts)}</span>
                      </div>
                      {e.prompt && (
                        <p className="text-gray-300 text-xs italic truncate">"{e.prompt}"</p>
                      )}
                      {e.matched_policy && (
              ...
              <div className="space-y-5">
                {!inv ? (
                  ...
                  <>
                    {/* WHO */}
                    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
                      <h3 className="text-sm font-bold text-indigo-400 uppercase tracking-wide mb-3">👤 WHO</h3>
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        <div className="bg-gray-800 rounded-lg p-3">
                          <p className="text-xs text-gray-400 mb-1">Agent</p>
                          <p className="text-white font-semibold">{inv?.who?.agent?.name || "—"}</p>
                          <p className="text-gray-500 text-xs capitalize">{inv?.who?.agent?.agent_type}</p>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-3">
                          <p className="text-xs text-gray-400 mb-1">Asset</p>
                          <p className="text-white font-semibold">{inv?.who?.asset?.name || "—"}</p>
                          <p className="text-gray-500 text-xs capitalize">{inv?.who?.asset?.classification}</p>
                        </div>
                      </div>
                      {(inv?.who?.end_users || []).length > 0 && (
                        <div>
                          <p className="text-xs text-gray-400 mb-2">End Users Involved</p>
                          {inv.who.end_users.map((eu: any, i: number) => (
                            <div key={eu.id || i} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2 mb-1">
                              <FiUser className="w-3.5 h-3.5 text-blue-400" />
                              <span className="text-blue-400 text-sm">{eu.email || eu.external || eu}</span>
                              {eu.ip && <span className="text-gray-500 text-xs ml-auto font-mono">{eu.ip}</span>}
                            </div>
                          ))}
                        </div>
                      )}
                    ...
                    {/* WHAT */}
                    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
                      <h3 className="text-sm font-bold text-orange-400 uppercase tracking-wide mb-3">⚡ WHAT</h3>
                      ...
                      {inv?.what?.events?.find((e: any) => e.decision === "deny")?.prompt && (
                        <div className="bg-gray-800 border border-gray-700 rounded-lg p-3">
                          <p className="text-xs text-gray-400 mb-1">Triggering Prompt</p>
                    ...
                    {/* WHY */}
                    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
                      <h3 className="text-sm font-bold text-red-400 uppercase tracking-wide mb-3">🚨 WHY</h3>
                      {inv?.why?.reason && (
                      ...
                      {(inv?.why?.policies_applied || []).map((p: any, i: number) => (
                        <div key={p.id || i} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2 mb-1">
                          <FiShield className="w-3.5 h-3.5 text-orange-400" />
                          <span className="text-white text-sm">{p.name}</span>
                          <span className="text-xs text-gray-500 ml-auto">Priority {p.priority}</span>
                        </div>
                      ))}
                    </div>
                  </>
            ...
            <div className="space-y-2">
              {auditTrail.length === 0
                ...
                : auditTrail.map((al: any, i: number) => (
                    <div key={al.id || i} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2.5">
                      <div className="flex items-center justify-between">
                        <span className="text-white text-sm font-medium">{al.action?.replace(/_/g," ")}</span>
                        <span className="text-gray-500 text-xs font-mono">{fmt(al.ts)}</span>
                      </div>
                      {Object.keys(al.changes || {}).length > 0 && (
                        <pre className="text-xs text-gray-400 mt-1 overflow-x-auto bg-gray-800 rounded px-2 py-1">
      ...
      <main className="min-h-screen bg-gray-950 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={load}
              className="flex items-center gap-2 bg-gray-800 border border-gray-700 text-gray-300 px-4 py-2 rounded-lg text-sm hover:bg-gray-700 transition-colors">
          ...
          <div className="flex gap-2 mb-5 flex-wrap">
            <button onClick={() => setStatusFilter("")}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                !statusFilter ? "bg-indigo-600 border-indigo-500 text-white" : "bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500"
              }`}>All ({incidents.length})</button>
            {STATUSES.map(s => (
              <button key={s} onClick={() => setStatusFilter(statusFilter === s ? "" : s)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                  statusFilter === s ? "bg-gray-700 border-gray-500 text-white" : "bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500"
                }`}>
          ...
          <div className="flex gap-3 mb-5">
            <div className="flex-1 flex items-center gap-2 bg-gray-900 border border-gray-700 rounded-xl px-4 py-2.5">
            ...
            <select value={sevFilter} onChange={e => setSevFilter(e.target.value)}
              className="bg-gray-900 border border-gray-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500">
              ...
              {filtered.length === 0
                ? (
                  <div className="bg-gray-900 border border-gray-700 rounded-xl p-10 text-center">
                    ...
                    <p className="text-gray-500 font-medium">No incidents match the current filters.</p>
                ...
                : filtered.map(incident => (
                    <div key={incident.id}
                      ...
                      className="bg-gray-900 border border-gray-700 rounded-xl p-4 cursor-pointer hover:border-gray-500 hover:bg-gray-900/80 transition-all">
                        ...
                        <div className="flex items-start gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                              {incident.agent_name && (
                                <span className="flex items-center gap-1 text-xs text-gray-400">🤖 {incident.agent_name}</span>
                              )}
                              {incident.asset_name && (
                                <span className="flex items-center gap-1 text-xs text-gray-400">🗄 {incident.asset_name}</span>
                              )}
                              {incident.owner && (
