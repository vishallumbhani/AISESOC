...
const RuntimePage: React.FC = () => {
  ...
  const CategoryBadge = ({ cat }: { cat: string | null }) => {
    ...
    const map: Record<string, string> = {
      ...
      general_query:        "bg-gray-100 text-gray-600",
    };
    return (
      <span className={`px-1.5 py-0.5 rounded text-xs ${map[cat] || "bg-gray-100 text-gray-600"}`}>
        {cat.replace(/_/g, " ")}
      </span>
    );
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-100 text-gray-600"
            >
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl border border-red-200 p-8 text-center">
              ...
              <button onClick={() => fetch(offset)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">Retry</button>
            </div>
          ) : events.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              <FiActivity className="w-12 h-12 mx-auto mb-4 text-gray-200" />
                  ...
                  {events.map((e) => (
                    <React.Fragment key={e.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-gray-500">{e.action}</td>
                        ...
                        <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{formatTime(e.created_at)}</td>
                        <td className="px-4 py-3 text-gray-400 text-xs">{expanded === e.id ? "▲" : "▼"}</td>
                      </tr>
                      {expanded === e.id && (
