...
const SystemHealth: React.FC = () => {
  ...
  const Icon = ({ status }: { status: string }) => {
    if (status === "healthy") return <FiCheckCircle className="w-5 h-5 text-green-400" />;
    if (status === "degraded") return <FiAlertCircle className="w-5 h-5 text-red-400" />;
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    const map: Record<string, string> = {
      ...
      degraded: "bg-red-900/40 text-red-400 border border-red-800",
        ...
        <div className="max-w-2xl mx-auto px-4">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <button onClick={check} disabled={loading}
              className="ml-auto flex items-center gap-1.5 px-3 py-2 bg-gray-800 border border-gray-700 text-gray-400 hover:text-white rounded-lg text-sm disabled:opacity-50">
          ...
          <div className={`rounded-xl border p-4 mb-5 flex items-center gap-3 ${
            ...
            overall === "degraded" ? "bg-red-900/20 border-red-800"    : "bg-gray-800 border-gray-700"
          }`}>
            <Icon status={overall} />
            <div>
              ...
              <p className="text-gray-500 text-xs mt-0.5">AI-SecOS Platform v1.0.0</p>
            </div>
          </div>
