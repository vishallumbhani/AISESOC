...
function TraceRow({ entry, index }: { entry: SimulateTraceEntry; index: number }) {
  ...
  return (
    <div className={`flex items-start gap-3 py-2.5 border-b border-gray-800 last:border-0 text-sm ${
      matchedDeny ? "bg-red-900/20" : matchedAllow ? "bg-green-900/20" : ""
    }`}>
      <span className="w-6 h-6 flex items-center justify-center rounded-full bg-gray-800 text-xs font-bold text-gray-500 flex-shrink-0 mt-0.5">
        {index + 1}
      </span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="font-medium text-gray-200 truncate">{entry.policy}</span>
          {entry.matched && entry.effect && (
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
              matchedDeny ? "bg-red-900/60 text-red-400" : "bg-green-900/60 text-green-400"
            }`}>{entry.effect.toUpperCase()}</span>
          )}
          {!entry.matched && <span className="text-xs text-gray-600">no match</span>}
        </div>
        {entry.rule && entry.matched && (
          <p className="text-xs text-gray-500 font-mono mt-0.5 truncate">{entry.rule}</p>
        )}
      </div>
      <div className="flex-shrink-0">
        {entry.matched
          ...
          : <span className="w-4 h-4 block rounded-full border border-gray-700" />}
      </div>
    </div>
  );
}
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="mb-8 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">Dry-run decisions before they affect real traffic. No events are written.</p>
            </div>
          </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
                ...
                <form onSubmit={handleSimulate} className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <label htmlFor="customRules" className="text-sm text-gray-400 cursor-pointer">
                      Test custom rules (bypass stored policies)
                    </label>
                  </div>
                  {useCustomRules && (
                    <div>
                      <label className="block text-xs font-medium text-gray-500 mb-1.5">Custom Rules JSON</label>
                      <textarea value={customRules} onChange={e => setCustomRules(e.target.value)}
                        className="w-full bg-gray-800 border border-gray-700 text-gray-200 font-mono text-xs rounded-lg px-3 py-2"
                        rows={6} />
                    </div>
                  )}
                  <button type="submit" disabled={!agentId || !assetId || testing}
                    className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold rounded-lg text-sm transition flex items-center justify-center gap-2">
                ...
                <div className="border-t border-gray-800 pt-4 flex items-start gap-2 text-xs text-gray-600">
              ...
              <div className="space-y-4">
                {result ? (
                  <>
                    ...
                    <div className={`rounded-xl border-2 p-5 ${
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Simulated Decision</p>
                          <p className={`text-4xl font-black ${isAllow ? "text-green-400" : "text-red-400"}`}>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
                      {result.matched_policy && (
                        <div>
                          <p className="text-xs text-gray-500 font-medium mb-0.5">Matched Policy</p>
                          <p className="text-white font-semibold">{result.matched_policy}</p>
                        </div>
                      )}
                      {result.matched_rule && (
                        <div>
                          <p className="text-xs text-gray-500 font-medium mb-0.5">Matched Rule</p>
                          <p className="text-gray-300 font-mono text-sm bg-gray-800 rounded px-2 py-1">{result.matched_rule}</p>
                        </div>
                      )}
                      <div>
                        <p className="text-xs text-gray-500 font-medium mb-0.5">Explanation</p>
                        <p className="text-gray-300 text-sm leading-relaxed">{result.explanation}</p>
                      </div>
                      {result.risk_score != null && (
                        <div>
                          <p className="text-xs text-gray-500 font-medium mb-0.5">Asset Risk Score</p>
                          <p className="text-gray-300 font-semibold">{result.risk_score}</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                      <button onClick={() => setShowTrace(!showTrace)}
                        className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold text-gray-300 hover:bg-gray-800">
                      ...
                      {showTrace && (
                        <div className="px-4 pb-3">
                          {!result.trace?.length
                            ? <p className="text-gray-600 text-sm py-2">No policies evaluated.</p>
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-10 text-center text-gray-600">
                    ...
                    <p className="font-medium text-gray-500">Configure and run a simulation</p>
