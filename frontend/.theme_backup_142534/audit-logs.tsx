...
const AuditLogsPage: React.FC = () => {
  ...
  const analyticsCards = analytics ? [
    { label: "Total Events",    value: analytics.total         ?? 0, color: "text-blue-400",   icon: <FiClipboard className="w-4 h-4" /> },
    { label: "Policy Changes",  value: analytics.policy_changes ?? 0, color: "text-green-400",  icon: <FiShield className="w-4 h-4" /> },
    { label: "Access Denials",  value: analytics.access_denials ?? 0, color: "text-red-400",    icon: <FiAlertTriangle className="w-4 h-4" /> },
    { label: "Incident Updates",value: analytics.incident_updates ?? 0, color: "text-orange-400", icon: <FiActivity className="w-4 h-4" /> },
  ] : [];
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-950 py-8">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-500 text-xs mt-0.5">Immutable record of all platform actions</p>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={handleDownload} disabled={downloading}
                className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm disabled:opacity-50">
          ...
          {/* Filters */}
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-4 flex flex-wrap gap-3 items-end">
            <form onSubmit={handleSearch} className="flex gap-2">
              <div className="relative">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)}
                  ...
                  className="pl-8 bg-gray-800 border border-gray-700 text-gray-200 placeholder-gray-600 rounded-lg px-3 py-1.5 text-sm w-56 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
              <button type="submit" className="px-3 py-1.5 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700">Search</button>
            </form>
          ...
          {loading ? (
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
                <span className="text-gray-400 text-sm">{filtered.length.toLocaleString()} entries</span>
              </div>
                        ...
                        <React.Fragment key={log.id}>
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-gray-400 text-xs">
                              {changes.agent && <div className="text-gray-300">🤖 {changes.agent}</div>}
                              {changes.asset && <div className="text-gray-500">🗄️ {changes.asset}</div>}
                              {!changes.agent && !changes.asset && <span className="text-gray-600">{log.resource_type || "—"}</span>}
                            </td>
                            <td className="px-4 py-3">
                              {decision ? (
                                <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${DECISION_CLASSES[decision] || "text-gray-400"}`}>
                                  {decision}
                                </span>
                              ) : "—"}
                            </td>
                            <td className="px-4 py-3">
                              {corrId ? (
                                <span className="font-mono text-xs bg-blue-950 text-blue-400 px-1.5 py-0.5 rounded">
                                  {corrId}
                                </span>
                              ) : "—"}
                            </td>
                            <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{fmt(log.created_at)}</td>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                                  {Object.entries(changes).filter(([, v]) => v).map(([k, v]) => (
                                    <div key={k} className="bg-gray-800 rounded-lg px-3 py-2">
                                      ...
                                      <p className="text-gray-200 font-mono break-all">{String(v)}</p>
                                    </div>
                                  ))}
                                  <div className="bg-gray-800 rounded-lg px-3 py-2">
                                    <p className="text-gray-500 mb-0.5">Log ID</p>
                                    <p className="text-gray-600 font-mono text-xs">{log.id}</p>
                                  </div>
                                </div>
                              </td>
                            </tr>
