    ...
    <div className="flex items-end gap-1 h-24">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
          ...
          <span className="text-gray-600 text-[9px] whitespace-nowrap">{d.date.replace(/\w+ /, "")}</span>
        </div>
      ))}
    </div>
  );
...
function SectionHeader({ title, href, icon }: { title: string; href?: string; icon?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between mb-3">
      ...
      {href && (
        <Link href={href} className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-0.5">
...
function ComplianceArc({ score, name }: { score: number; name: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <p className="text-gray-500 text-[10px] text-center leading-tight">{name}</p>
    </div>
  );
}
...
const Dashboard: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">Loading enterprise dashboard…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300 mb-4">{error || "No data"}</p>
  ...
  const trendColor = overview.risk_trend > 0 ? "text-red-400" : overview.risk_trend < 0 ? "text-green-400" : "text-gray-400";
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-950 pb-10">
        {/* Top bar */}
        <div className="border-b border-gray-800 bg-gray-900/80 backdrop-blur sticky top-0 z-20 px-6 py-2 flex items-center justify-between">
          ...
          <button onClick={load}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 border border-gray-700 text-gray-400 hover:text-white rounded-lg text-xs">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 pt-5 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
            ...
            <div className={`col-span-2 md:col-span-1 border rounded-xl p-4 ${RISK_LEVEL_CLS[overview.risk_level] || "border-gray-700 bg-gray-900"}`}>
              <p className="text-xs text-gray-400 uppercase tracking-wide mb-2">Enterprise Risk Score</p>
            ...
            {/* 5 KPI cards */}
            {[
              { label: "AI Assets Protected",         value: overview.total_assets,  icon: <FiDatabase />, href: "/assets",          accent: "text-blue-400" },
              { label: "AI Agents Protected",          value: overview.total_agents,  icon: <FiCpu />,     href: "/agents",          accent: "text-purple-400" },
              { label: "Active Security Policies",     value: overview.total_policies, icon: <FiShield />, href: "/policies",        accent: "text-indigo-400" },
              { label: "High Risk AI Assets",          value: high_risk_assets.length, icon: <FiAlertTriangle />, href: "/assets?severity=high", accent: high_risk_assets.length > 0 ? "text-orange-400" : "text-gray-500" },
              { label: "Active AI Security Incidents", value: overview.open_incidents, icon: <FiTarget />, href: "/incidents?status=open", accent: overview.open_incidents > 0 ? "text-red-400" : "text-gray-500" },
            ].map(card => (
              <Link key={card.label} href={card.href}>
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-gray-600 transition-all cursor-pointer h-full">
          ...
          <Card>
            ...
            <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
              {[
                { label: "AI Requests",       value: fmt(today.total),            color: "text-blue-400" },
                { label: "Blocked",           value: fmt(today.blocked),           color: "text-red-400" },
                { label: "Allowed",           value: fmt(today.allowed),           color: "text-green-400" },
                { label: "New Incidents",     value: today.new_incidents,          color: "text-orange-400" },
                { label: "Critical Prompts",  value: today.critical_prompts,       color: "text-red-400" },
                { label: "Policy Violations", value: today.policy_violations,      color: "text-yellow-400" },
                { label: "Unique Users",      value: today.unique_users,           color: "text-purple-400" },
                { label: "Unique Agents",     value: today.unique_agents,          color: "text-indigo-400" },
              ].map(m => (
                <div key={m.label} className="text-center">
                  <p className={`text-2xl font-bold ${m.color}`}>{m.value}</p>
                  <p className="text-gray-600 text-[10px] mt-0.5 leading-tight">{m.label}</p>
                </div>
              ))}
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              {high_risk_assets.length === 0 ? (
                <div className="py-6 text-center text-gray-600 text-sm">No high-risk assets detected. ✓</div>
              ) : (
                    ...
                    <div key={a.id} className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          <span className="text-gray-200 text-sm font-medium truncate">{a.name}</span>
                      ...
                      {a.trend === "up"
                        ? <FiTrendingUp className="w-3.5 h-3.5 text-red-400 flex-shrink-0" />
                        : <FiTrendingDown className="w-3.5 h-3.5 text-green-400 flex-shrink-0" />}
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              {high_risk_agents.length === 0 ? (
                <div className="py-6 text-center text-gray-600 text-sm">No high-risk agents this month. ✓</div>
              ) : (
                <div className="space-y-2.5">
                  {high_risk_agents.map((a: any, i: number) => (
                    <div key={a.id} className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-red-400 flex-shrink-0">{a.denials} denials</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <Card>
              ...
              {prompt_threats.length === 0 ? (
                <div className="py-6 text-center text-gray-600 text-sm">No threat categories detected this week.</div>
              ) : (
                <div className="space-y-2.5">
                  {prompt_threats.map((t: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <span className="text-gray-200 text-sm flex-1">{t.category}</span>
                        <div className="w-24 bg-gray-800 rounded-full h-1.5">
                          <div className="h-1.5 rounded-full bg-orange-500" style={{ width: `${(t.count / max) * 100}%` }} />
                        </div>
                        <span className="text-orange-400 text-sm font-bold w-6 text-right">{t.count}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <Link href="/incidents" className="block text-center text-xs text-indigo-400 hover:text-indigo-300 mt-1">
                Open Incident Queue →
              </Link>
            </Card>
            ...
            <Card>
              ...
              {policy_effectiveness.length === 0 ? (
                <div className="py-6 text-center text-gray-600 text-sm">No policies triggered yet today.</div>
              ) : (
                <div className="space-y-2.5">
                  {policy_effectiveness.map((p: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <span className="text-gray-200 text-sm flex-1 truncate">{p.policy}</span>
                        <div className="w-24 bg-gray-800 rounded-full h-1.5">
                          <div className="h-1.5 rounded-full bg-indigo-500" style={{ width: `${(p.triggers / max) * 100}%` }} />
                        </div>
                        <span className="text-indigo-400 text-sm font-bold w-6 text-right">{p.triggers}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          <Card>
            ...
            {timeline.length === 0 ? (
              <div className="py-6 text-center text-gray-600 text-sm">No events yet. Configure a connector to start.</div>
            ) : (
                  ...
                  {timeline.slice(0, 12).map((e: any, i: number) => (
                    <div key={e.id || i} className={`border rounded-xl p-3 flex-shrink-0 w-44 ${
                      ...
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-gray-500 text-xs font-mono">{e.time}</span>
                        {e.decision === "deny"
                          ? <FiXCircle className="w-3.5 h-3.5 text-red-400" />
                          : <FiCheckCircle className="w-3.5 h-3.5 text-green-400" />}
                      </div>
                      <p className="text-white text-xs font-semibold truncate">🤖 {e.agent}</p>
                      <p className="text-gray-400 text-xs truncate mt-0.5">→ {e.asset}</p>
                      {e.category && e.category !== "general_query" && (
                        <span className="text-[10px] text-orange-400 mt-1 block truncate">{e.category.replace(/_/g, " ")}</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <Card>
              ...
              {connector_health.length === 0 ? (
                <div className="py-4 text-center">
                  <p className="text-gray-600 text-sm mb-2">No connectors configured yet.</p>
                  <Link href="/enterprise" className="text-xs text-indigo-400 hover:text-indigo-300">
                ...
                <div className="space-y-2">
                  {connector_health.map((c: any) => (
                    <div key={c.id} className="flex items-center gap-3 bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-2">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${c.status === "healthy" ? "bg-green-400" : "bg-red-400"}`} />
                      <span className="text-gray-200 text-sm font-medium flex-1">{c.name}</span>
                      <span className={`text-xs font-semibold ${c.status === "healthy" ? "text-green-400" : "text-red-400"}`}>
                        {c.status === "healthy" ? "Healthy" : "Error"}
                      </span>
                      <span className="text-gray-500 text-xs">{c.requests_today} reqs</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <Link key={item.label} href={item.href}>
                    <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-3 text-center hover:border-gray-500 transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <Card className="lg:col-span-2">
              ...
              {compliance_scores.length === 0 ? (
                <div className="py-4 text-gray-600 text-sm text-center">Compliance data loading…</div>
              ) : (
              ...
              <div className="mt-3 space-y-1">
                {Object.entries(governance.breakdown).map(([key, val]: [string, any]) => (
                  <div key={key} className="flex items-center gap-2 text-xs">
                    <span className="text-gray-500 flex-1 capitalize">{key.replace("_", " ")}</span>
                    <div className="w-16 bg-gray-800 rounded-full h-1">
                      <div className="h-1 rounded-full bg-indigo-500" style={{ width: `${val}%` }} />
                    </div>
                    <span className="text-gray-400 w-8 text-right">{val}%</span>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
              {recommendations.map((r: any, i: number) => (
                <Link key={i} href={r.href || "#"}>
                  <div className="flex items-center gap-3 bg-gray-800/50 border border-gray-700 hover:border-indigo-700 hover:bg-indigo-900/10 rounded-xl px-4 py-3 transition-all cursor-pointer">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${PRIO_DOT[r.priority] || "bg-gray-500"}`} />
                    <span className="text-gray-200 text-sm flex-1">{r.action}</span>
                    <FiArrowRight className="w-3.5 h-3.5 text-gray-600 flex-shrink-0" />
                  </div>
                </Link>
              ))}
            </div>
          ...
          <Card className="border-indigo-800/60 bg-indigo-950/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <p className="text-xs text-indigo-400 uppercase font-semibold mb-2">Security Summary — Today</p>
              ...
              <div>
                <p className="text-xs text-indigo-400 uppercase font-semibold mb-2">Recommended Actions</p>
                <ul className="space-y-1.5">
                  {ai_summary.recommendations.map((line: string, i: number) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-gray-300">
                      <span className="text-orange-400 mt-0.5 flex-shrink-0">{i + 1}.</span> {line}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="flex flex-wrap gap-4">
              {[
                ...
                <div key={label} className="flex items-center gap-2">
                  ...
                  <span className={`text-xs font-medium ${s === "Healthy" ? "text-green-400" : s === "Unknown" ? "text-gray-500" : "text-red-400"}`}>{s || "Checking…"}</span>
                </div>
              ))}
            </div>
          </Card>
