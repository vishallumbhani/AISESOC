...
async def graph_live_events(
    ...
    """Recent runtime events for the live event feed table."""
    ...
    # Fetch raw metadata JSONB via SQL to avoid SQLAlchemy MetaData conflict
    if events:
        event_ids = [str(e.id) for e in events]
        ids_sql = ", ".join(f"'{eid}'" for eid in event_ids)
        meta_rows = db.execute(sql_text(
            f"SELECT id::text, metadata FROM runtime_events WHERE id IN ({ids_sql})"
        )).fetchall()
        meta_map = {row[0]: (row[1] or {}) for row in meta_rows}
    else:
        meta_map = {}
    ...
    for e in events:
        ...
        agent_name = (agent_map.get(e.agent_id)
                      or meta.get("connector")
                      or meta.get("agent_name")
                      or "Unknown")
        asset_name = asset_map.get(e.asset_id) or "Unknown"
        result.append({
            "id":              eid,
            ...
            "risk_score":      meta.get("prompt_risk_score"),
        })
    return result
